...
(async () => {
  ...
  app.get( "/cars", ( req: Request, res: Response ) => {
      ...
      if (make) {
        let filteredCars = cars_list.filter(car => car.make == make);
  ...
  app.get( "/cars/:id", ( req: Request, res: Response ) => {
    ...
    let foundCar = cars_list.find(car => car.id == parseInt(id))
    if (!foundCar) {
  ...
  app.post("/cars", async (req: Request, res: Response) => {
    ...
    if (!id || !type || !make || !model || !cost) {
      return res.status(400).send({message: 'id, type, make, model and cost are required'});
    }

    const newCar: Car = {
      id: id, type: type, make: make, model: model, cost: cost
    }
